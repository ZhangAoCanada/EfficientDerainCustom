...
def get_files(path):
    ret = []
    #path_rainy = path + "\\train\\data_new"
    #path_gt = path + "\\train\\gt_new"

    path_rainy = path + "/trainval1/rain"
    path_gt = path + "/trainval1/gt"

    #path_rainy = path + "/test/data_new"
    #path_gt = path + "/test/gt_new"
    ...
    for root, dirs, files in os.walk(path_rainy):
        ...
        for name in files:
            ...
            #file_gt = path_gt + "\\" + name.replace('_rain.', '_clean.')
            file_gt = path_gt + "/" + name.replace('Rain_', 'No_Rain_')
            print(file_rainy)
            print(file_gt)
