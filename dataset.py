...
class RandomCrop(object):
    def __init__(self, image_size, crop_size_h, crop_size_w):
        self.ch, self.cw = crop_size_h, crop_size_w
        ih, iw = image_size
...
class DenoisingDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        cropper = RandomCrop(img_gt.shape[:2], self.opt.crop_size_h, self.opt.crop_size_w)
...
class DenoisingValDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.crop:
            ...
            cropper = RandomCrop(img_rainy.shape[:2], self.opt.crop_size_h, self.opt.crop_size_w)
            img_rainy = cropper(img_rainy)
            img_gt = cropper(img_gt)
        # random rotate and horizontal flip
        # according to paper, these two data augmentation methods are recommended
